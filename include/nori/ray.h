...
struct TRayDifferential : TRay <typename _PointType, typename _VectorType> {
    using Base = TRay <typename _PointType, typename _VectorType>;
    typedef _PointType                  PointType;
    typedef _VectorType                 VectorType;
    typedef typename PointType::Scalar  Scalar;
    ...
    TRayDifferential(const Base& ray) : Base(ray), o_x(0), o_y(0), d_x(0), d_y(0), hasDifferential(false) {}
